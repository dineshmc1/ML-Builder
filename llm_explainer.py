...
def generate_comprehensive_report(master_context, dataset_id):
    ...
    (Explain how the system decided to approach the problem. Mention the similarity to past experience, consistency, and whether memory or LLM intuition drove the choice. Do not show math formulas, just explain the meaning of the scores).
    
    # 3. Model Selection & Hyperparameter Optimization (HPO)
    (Explain which models were tested, which were dropped and why, and the final winning model. Detail the exact HPO parameters found and what they mean for this specific model).
    ...
    (Explain the top 3 driving features. Give a business-friendly hypothesis on WHY these features impact the prediction based on their names).
