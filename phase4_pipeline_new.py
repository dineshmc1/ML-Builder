...
def load_local_dataset(csv_path, target_column):
    """
    Helper function to load dataset from a local CSV.
    Returns X (DataFrame), y (Series).
    """
    try:
        print(f"  -> Loading local dataset: {csv_path}...")
        data = pd.read_csv(csv_path)
        
        if target_column not in data.columns:
            print(f"  -> Target column '{target_column}' not found in {csv_path}. Skipping.")
            return None, None
...
def main():
    ...
    if remaining_train:
        store = build_memory(remaining_train, store, config=config)
